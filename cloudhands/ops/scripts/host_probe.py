...
def checkStoragePaths(host):
    ...
    for num in xrange(1,10):
        if len(arr[num]) != 12:
            print(
                "The number of paths available on host: ",
                host.FullName,
                " is less than 12. This is incorrect and should be resolved.")
        for a in arr[num]:
            if re.match(r"Group State: active$", a[1]):
                count1 = count1 + 1
        if count1 < 3:    
            print(
                "The number of ACTIVE paths available on host: ",
                host.FullName,
                " is less than 3. This is incorrect and should be resolved.")
